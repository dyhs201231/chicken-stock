...
export default function AccountData({ initialPortfolio }: AccountDataProps) {
  ...
  return (
    <div className="col gap-7">
      <div className="w-fit rounded-lg border border-(--cs-border-subtle) bg-(--cs-surface-raised) px-3 py-2 text-sm font-semibold text-(--cs-text-muted) shadow-(--cs-shadow-sm)">
        계좌 {data.accountNumber}
      </div>

      <div className="row flex-wrap items-end justify-between gap-6">
        <div>
          <p className="cs-section-label mb-2">Total assets</p>
        ...
        <div className="row flex-wrap gap-3">
          <Link href="/edu" className="row center min-h-10 cursor-pointer rounded-lg border border-(--cs-border-strong) bg-(--cs-surface-raised) px-4 font-semibold text-(--cs-brand-800) shadow-(--cs-shadow-sm) transition hover:bg-(--cs-brand-50)">
            퀴즈 풀고 충전하기
          </Link>
